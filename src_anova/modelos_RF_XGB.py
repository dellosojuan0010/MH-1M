...
def treina_modelo(pasta_saida, modelo, X_train, X_test, y_train, nome_base, salvar=False):
...
def definir_modelos_sklearn(input_dim):
    if input_dim > 20000:
        ...
        rf = RandomForestClassifier(n_estimators=150, max_depth=17, random_state=42)
        xgb = XGBClassifier(n_estimators=175, max_depth=9, learning_rate=0.05, verbosity=1, use_label_encoder=False, random_state=42)
    elif input_dim > 1000:
        rf = RandomForestClassifier(n_estimators=125, max_depth=16, random_state=42)
        xgb = XGBClassifier(n_estimators=160, max_depth=8, learning_rate=0.05, verbosity=1, use_label_encoder=False, random_state=42)
    elif input_dim > 400:
...
pasta_saida = os.path.join(".",nome_grupo,"SHAP",modelo_nome,"resultados")
...
arquivo_features_importantes = os.path.join(CAMINHO_ARQUIVO_RANK,f"importantes_unicas_{modelo_nome}_{nome_grupo}_shap_folds.csv")

import csv

idx_features = []
with open(arquivo_features_importantes, "r", newline="") as f:
    reader = csv.reader(f)
    next(reader)  # pula o cabeçalho ("idx")
    for linha in reader:
        if linha:  # evita linhas vazias
            idx_features.append(int(linha[0]))
