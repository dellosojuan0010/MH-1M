...
def definir_modelos_sklearn(input_dim):
    if input_dim > 20000:
        ...
        xgb = XGBClassifier(n_estimators=180, max_depth=9, learning_rate=0.06, verbosity=1, use_label_encoder=False, random_state=42)
    elif input_dim > 400:
...
for modelo_nome in modelos:
    ...
    for nome_grupo in grupos:
        ...
        caminho_ranking = os.path.join(".",nome_grupo,"ranking_features_fscore_pvalues.csv")
        df = pd.read_csv(caminho_ranking)

        
        #df_filtrado = df[df['f_score'] > 0]
        df_filtrado = df.head(qtde_features[k])
        k=k+1
        features_filtradas = df_filtrado['feature'].tolist()

        mask = np.isin(colunas, features_filtradas)

        X_fscore = X[:, mask]
        colunas_fscore = colunas[mask]

        df_final = pd.DataFrame(X_fscore, columns=colunas_fscore)
        df_final['classe'] = y
