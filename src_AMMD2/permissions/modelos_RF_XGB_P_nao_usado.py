...
def avaliar_modelos_em_dataframe(df, nome_grupo, pasta_saida, n_splits=5):
    ...
    for modelo_nome, modelo in tqdm(modelos.items(), desc=f"[{nome_grupo}] Modelos sklearn"):
        ...
        for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), start=1):
            ...
            caminho_cm_png = salvar_matrix_confusao(nome_grupo, pasta_cm, modelo_nome, fold, nome_base, cm)
...
def salvar_matrix_confusao(nome_grupo, pasta_cm, modelo_nome, fold, nome_base, cm):
...
pasta_saida = os.path.join("..", "..", "resultadosAMMD2", "pre_explicabilidade", f"resultado_RF_XGB_P_18092025")
